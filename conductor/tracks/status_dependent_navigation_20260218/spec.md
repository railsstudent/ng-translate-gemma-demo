...
### 2. TranslateComponent (Defensive Programming)
...
-   **Template:**
    ...
    -   Pass the full `modelsService.status()` object to the `TranslateComponent` (via router outlet input binding).
...
-   `TranslateComponent` correctly falls back to displaying the `StatusComponent` if models are not ready.
-   `TranslateComponent` shows the "coming soon" stub only when the status is `success`.
